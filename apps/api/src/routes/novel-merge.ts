...
export const novelMergeRoutes = new Hono<AuthEnv>()
  .use("*", requireAuth)
  /**
   ...
   * guarded against what the bible already holds. Deliberately not wrapped in
   * one transaction: a merge is a list of independent decisions, and the author
   * is told exactly which ones landed. Failing the good ones because a later
   * one was a duplicate would be worse than partial application.
   */
  .post("/:novelId/merge", zValidator("json", mergeSchema), async (c) => {
    ...
    const characterIds = new Set(existingChars.map((ch) => ch.id));
    ...
    // row — overwriting would erase the state the bond had for the chapters
    // before this one, which is the whole thing this table exists to keep.
    ...
    for (const rel of newRelationships) {
      ...
      // the two writes go together. This used to be an insert plus a
      // compensating delete; the driver can hold a real transaction now.
      try {
